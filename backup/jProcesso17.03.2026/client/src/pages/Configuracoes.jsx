...
export default function Configuracoes() {
  ...
  const handleExport = async (tipo, formato = 'json') => {
    ...
    try {
      ...
      const resp = await api.get(`/api/export/${tipo}?formato=${formato}`, {
        responseType: 'blob',
  ...
  const handleImport = async () => {
    ...
    try {
      const api = (await import('../services/api')).default;
      const resp = await api.post('/api/import', formData, {
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={path} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700">
